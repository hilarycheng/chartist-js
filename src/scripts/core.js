 ...
 * @return {String}
 */
let replaceAll = (str, subStr, newSubStr) => {
 ...
 * @return {Array}
 */
let serialMap = (arr, cb) => {
  let result = [],
    length = Math.max.apply(null, arr.map((e) => {
      return e.length;
    }))

  times(length).forEach((e, index) => {
    let args = arr.map((e) => {
      return e[index];
    })
 ...
 */
let roundWithPrecision = (value, digits) => {
  let _precision = Math.pow(10, digits || precision);
...
let serialize = (data) => {
  ...
  return Object.keys(escapingMap).reduce((result, key) => {
    return replaceAll(result.toString(), key, escapingMap[key]);
  }, data).toString();
}
...
let deserialize = (data) => {
  ...
  data = Object.keys(escapingMap).reduce((result, key) => {
 ...
 */
let createSvg = (container, width, height, className) => {
  let svg;
 ...
 */
let normalizeData = (data, reverse, multi) => {
  let labelCount;
  let output = {
    raw: data,
    normalized: {}
 ...
 */
let safeHasProperty = (object, property) => {
  return object !== null &&
 ...
 */
let getDataArray = (data, reverse, multi) => {
 ...
 */
let normalizePadding = (padding, fallback) => {
  fallback = fallback || 0;
 ...
 */
let projectLength = (axisLength, length, bounds) => {
 ...
 */
let getAvailableHeight = (svg, options) => {
  return Math.max((quantity(options.height).value || svg.height()) - (options.chartPadding.top + options.chartPadding.bottom) - options.axisX.offset, 0);
}
 ...
 */
let getHighLow = (data, options, dimension) => {
 ...
 * @returns {*}
 */
let getNumberOrUndefined = (value) => {
 ...
 * @returns {*}
 */
let getMultiValue = (value, dimension) => {
 ...
 */
let getBounds = (axisLength, highLow, scaleMinSpace, onlyInteger) => {
  let i,
  ...
  let length = projectLength(axisLength, bounds.step, bounds);
  let scaleUp = length < scaleMinSpace;
  let smallestFactor = onlyInteger ? rho(bounds.range) : 0;
  ...
  let safeIncrement = (value, increment) => {
  ...
  for (i = bounds.min; i <= bounds.max; i = safeIncrement(i, bounds.step)) {
    let value = roundWithPrecision(i);
 ...
 */
let createChartRect = (svg, options, fallbackPadding) => {
  ...
  let normalizedPadding = normalizePadding(options.chartPadding, fallbackPadding);
  ...
  let chartRect = {
    ...
    width: () => {
      return chartRect.x2 - chartRect.x1;
    },
    height: () => {
      return chartRect.y1 - chartRect.y2;
    }
  };
 ...
 * @param eventEmitter
 */
let createGrid = (position, index, axis, offset, length, group, classes, eventEmitter) => {
  let positionalData = {};
 ...
 * @param eventEmitter
 */
let createGridBackground = (gridGroup, chartRect, className, eventEmitter) => {
 ...
 * @param eventEmitter
 */
let createLabel = (position, length, index, labels, axis, axisOffset, labelOffset, group, classes, useForeignObject, eventEmitter) => {
  let labelElement;
  let positionalData = {};
  ...
  if (useForeignObject) {
    ...
    content.setAttribute('xmlns', namespaces.xhtml);
 ...
 * @returns {*}
 */
let getSeriesOption = (series, options, key) => {
 ...
 */
let optionsProvider = (options, responsiveOptions, eventEmitter) => {
  let baseOptions = extend({}, options),
    currentOptions,
    mediaQueryListeners = [],
    i;
 ...
 */
let splitIntoSegments = (pathCoordinates, valueData, options) => {
  let defaultOptions = {
    increasingX: false,
    fillHoles: false
  };
